...
// 일정 시간후에 func을 작업하라
// TODO::최초 딜레이 시간과 반복 딜레이 시간을 구분하라
...
void Timer::Start(function<void()> func, int startSec, int milliSec, UINT repeat)
{
	assert(bComplete == false); // 이미 작동 중이라면
	bComplete = false; // 작동중
	thread t([=]()
	{
		count++;
...
float Performance::End()
{
	...
	//return (float)((double)(end - start) / tick);
