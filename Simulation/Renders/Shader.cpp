...
void Shader::CreateEffect()
{
	...
	if (Path::GetExtension(file) == L"fx")// 기존 바식 대로 컴파일
	{
		ID3DBlob* error;
		//INT flag = D3DCOMPILE_SKIP_OPTIMIZATION | D3DCOMPILE_DEBUG | D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY | D3D10_SHADER_PACK_MATRIX_ROW_MAJOR;
		INT flag = D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY | D3D10_SHADER_PACK_MATRIX_ROW_MAJOR;

		D3D_SHADER_MACRO shader_macro = { 0 };
		HRESULT hr;
		//hr = D3DX11CompileEffectFromFile(file.c_str(), 0, D3D_COMPILE_STANDARD_FILE_INCLUDE, 0, 0, D3D::Get()->GetDevice(), &effect, &error);
		hr = D3DCompileFromFile(file.c_str(), &shader_macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, 0, "fx_5_0", flag, NULL, &fxBlob, &error);
		if (FAILED(hr))
		{
			if (error != NULL)
			{
	...
	else if (Path::GetExtension(file) == L"fxo") // 미리 컴파일 해놓은 파일 읽기
	{
...
ID3D11InputLayout * Shader::CreateInputLayout(ID3DBlob * fxBlob, D3DX11_EFFECT_SHADER_DESC* effectVsDesc, vector<D3D11_SIGNATURE_PARAMETER_DESC>& params)
{
	...
	for (D3D11_SIGNATURE_PARAMETER_DESC& paramDesc : params)
	{
		...
		// INST로 시작한다면 인스턴싱 슬롯이며
		// 다음의 숫자는 슬롯 번호가 될것입니다. 여러개의 슬롯을 넣을수가 있다.
		if (String::StartsWith(name, "INST") == true)
		{
			String::Replace(&name, "INST", "");
			string start = name.substr(0, 1); // 뒤에 숫자가 슬롯번호
